<template>
  ...
  -->
      ...
      <h3 slot="header" style="margin-bottom: 0px;">Приобретите курс, чтобы продолжить</h3>

      <form slot="body" action="/payment/check" method="get" class="payment" @submit="submitForm">

        <a class="button no-border" href="https://profguide.org/ru/#course">Приобрести</a>
      </form>

      <a slot="button"></a>

      <a class="modal-link" slot="back" href="https://profguide.org/ru">вернуться</a>
    </modal>
  </main>
</template>
    ...
    submitForm: function (e) {
      ...
      $.ajax({
        ...
        success: function (data) {
          if (data) {
            $('form').attr('action', 'https://profguide.org/ru/#course');
